...
let getViewForSwagger2 = function(opts) {
  ...
  _.forEach(swagger.paths, function(api, path) {
    ...
    _.forEach(api, function(op, m) {
      ...
      var methodNameCamelCase = _.camelCase(methodName)
      if (methodNameCamelCase.indexOf('Prototype') > 0) {
        methodNameCamelCase = _.replace(methodNameCamelCase, 'Prototype', '')
      }

      let model = op.operationId.split('.')[0]
      if(!swagger.definitions[model] || swagger.definitions[model]["x-codegen"] == false){
        return
      }
...
let parse = function(opts) {
  let data = getViewForSwagger2(opts)
  return data
}
